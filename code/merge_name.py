...
def merge_records(data):
    ...
    for index, record in enumerate(data):
        ...
        for info in record['infos']:
            name = info['基本信息-姓名'][0:3]
            if not name[2:3].isdigit():
                continue
        ...
        if first_valid_record:
            similar_infos = [first_valid_record] + [other_info for other_info in record['infos'] if
                                                    other_info['基本信息-姓名'] == first_valid_record['基本信息-姓名']]
            merged_info = (
...
with open('../user_data/try_test_data_v1.json', 'r', encoding='utf-8') as f:  # No change
    data = json.load(f)
...
with open('../user_data/merged_final_outputV1.json', 'w', encoding='utf-8') as f:  # No change
